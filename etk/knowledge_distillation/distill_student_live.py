...
def load_trainset_from_log(path, 
    ...
    # idx = 0
    # for task_id, entry in enumerate(log):
    #     print(task_id, idx)
...
def gptneo_data_collator(data):
    return {'input_ids': torch.stack([f["input_ids"] for f in data]),
            ...
            # 'idx': torch.stack([f["idx"] for f in data]),
            }
...
if teacher_name=="EleutherAI/gpt-j-6B":
    teacher.parallelize()
...
class DistilTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            t_logits = teacher(**inputs).logits[:,:,:,:50257]
...
# Runs training 
DistilTrainer(teacher=teacher, model=model, args=training_args, train_dataset=dataset,
